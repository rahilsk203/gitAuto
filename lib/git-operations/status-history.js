...
async function showCommitHistoryAdvanced(options = {}) {
  try {
    ...
    if (result.success) {
      // Check if there's actual commit history to show
      if (result.stdout && result.stdout.trim() !== '') {
        ...
        return { success: true, history: 'No commits found' };
      }
    } else {
      // Handle specific error cases
      if (result.stderr && result.stderr.includes('does not have any commits yet')) {
        console.log('📝 Commit History:');
        console.log('==================');
        console.log('No commits found in this repository.');
        return { success: true, history: 'No commits found' };
    ...
    // Even if there's an error, we want to return a success response to prevent the menu from breaking
    console.log('📝 Commit History:');
    console.log('==================');
    console.log('Unable to retrieve commit history. This repository may be empty or not initialized.');
